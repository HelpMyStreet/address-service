    ...
    public class GetNearbyPostcodes
    {
            ...
            ILogger log)
        {
            try
            {
                ...
                {
                    return new OkObjectResult(ResponseWrapper<GetNearbyPostcodesResponse>.CreateUnsuccessfulResponse("Invalid postcode"));
                }
                ...
                {
                    return new OkObjectResult(ResponseWrapper<GetNearbyPostcodesResponse>.CreateUnsuccessfulResponse(validationResults));
                }
            }
            catch (Exception exc)
            {
                log.LogError(exc, "Unhandled error in GetNearbyPostcodes");
                return new ObjectResult(ResponseWrapper<GetNearbyPostcodesResponse>.CreateUnsuccessfulResponse("Internal Error")) { StatusCode = StatusCodes.Status500InternalServerError };
            }
        }
    }
}
